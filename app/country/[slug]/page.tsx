...
import { ArrowLeft, Flag, MapPin, Shield, DollarSign, CloudSun } from "lucide-react";
...
export default function CountryPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`/api/country?name=${slug}`);
                if (!res.ok) throw new Error("Could not find country data");
                const json = await res.json();
                setData(json);

                // Only save to search history if this is a new search (not from clicking history)
                // Check for 'fromSearch' query parameter
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center">
                ...
                <p className="text-teal-400 text-lg animate-pulse">Consulting AI Guides...</p>
            </div>
        );
    }
